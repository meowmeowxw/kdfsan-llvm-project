...
bool DataFlowSanitizer::initializeModule(Module &M) {
  ...
  case Triple::aarch64:
    ...
    break;
  case Triple::x86_64:
    static MemoryMapParams LocalParams = Linux_X86_64_MemoryMapParams;
    if (ClEnableKdfsan) {
        LocalParams.XorMask = 0x0;
    }
    MapParams = &LocalParams;
  ...
  Type *DFSanLoadStoreCallbackArgs[4] =
      { Type::getInt8PtrTy(*Ctx), IntptrTy, PrimitiveShadowTy, PrimitiveShadowTy };
  DFSanLoadStoreCallbackFnTy = FunctionType::get(PrimitiveShadowTy,
  ...
  DFSanKasperBranchCheckFnTy =
      FunctionType::get(Type::getVoidTy(*Ctx), PrimitiveShadowTy, /*isVarArg=*/false);
                                                  ...
                                                  DFSanVarargWrapperFnTy);
  {
    ...
    AL = AL.addFnAttribute(M.getContext(),
                           Attribute::getWithMemoryEffects(
                               M.getContext(), MemoryEffects::readOnly()));
...
bool DataFlowSanitizer::runImpl(
  ...
  SmallPtrSet<Constant *, 1> PersonalityFns;
    ...
    if (!F.isIntrinsic() && !DFSanRuntimeFunctions.contains(&F) &&
        !LibAtomicFunction(F) &&
        &F != dyn_cast<Function>(DFSanReadLabelFn.getCallee()->stripPointerCasts()) &&
        &F != dyn_cast<Function>(DFSanKasperBranchCheckFn.getCallee()->stripPointerCasts())
        ) {
...
std::pair<Value *, Value *> DFSanFunction::loadShadowOriginSansLoadTracking(
  ...
  if (ClEnableKdfsan) {
    IRBuilder<> IRB(Pos);
    CallInst *FallbackCall =
        IRB.CreateCall(DFS.DFSanReadLabelFn,
                       {IRB.CreateBitCast(Addr, Type::getInt8PtrTy(*DFS.Ctx)),
                        ConstantInt::get(DFS.IntptrTy, Size)});
    FallbackCall->addRetAttr(Attribute::ZExt); // Changed from addAttribute
    return std::make_pair(
        FallbackCall,
        nullptr); // Return a pair to match the expected return type
  }
...
void DFSanFunction::storePrimitiveShadowOrigin(Value *Addr, uint64_t Size,
  ...
  if (AllocaInst *AI = dyn_cast<AllocaInst>(Addr)) {
    ...
    if (ClEnableKdfsan) {
      IRBuilder<> IRB(Pos);
      IRB.CreateCall(DFS.DFSanSetLabelFn,
                     {PrimitiveShadow,
                      IRB.CreateBitCast(Addr, Type::getInt8PtrTy(*DFS.Ctx)),
                      ConstantInt::get(DFS.IntptrTy, Size)});
      return;
...
void DFSanVisitor::visitStoreInst(StoreInst &SI) {
  ...
  Value *PrimitiveShadow;
  // Your commented code for ClCombinePointerLabelsOnStore here...
  ...
  if (InsertStoreCallback) {
    IRBuilder<> IRB(&SI);  // Create IRBuilder here
    Shadow = IRB.CreateCall(
        DFSF.DFS.DFSanStoreCallbackFn,
        {IRB.CreateBitCast(SI.getPointerOperand(),
                          Type::getInt8PtrTy(*DFSF.DFS.Ctx)),
         ConstantInt::get(DFSF.DFS.IntptrTy, Size),
         Shadow,
         PtrShadow});
  }
  ...
  // Use the new storePrimitiveShadowOrigin API instead of storeShadow
...
void DFSanVisitor::visitMemTransferInst(MemTransferInst &I) {
  ...
  if (!ClEnableKdfsan) {
    ...
    if (ClPreserveAlignment) {
      if (auto DestAlign = I.getDestAlign()) {
        MTI->setDestAlignment(Align(DestAlign.value().value() * DFSF.DFS.ShadowWidthBytes));
      }
      if (auto SrcAlign = I.getSourceAlign()) {
        MTI->setSourceAlignment(Align(SrcAlign.value().value() * DFSF.DFS.ShadowWidthBytes));
      }
    } else {
